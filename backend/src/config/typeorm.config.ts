...
export const buildTypeOrmOptions = (config: ConfigLike): DataSourceOptions => {
  ...
  return {
    ...
    port,
    username: config.get<string>('DB_USERNAME', 'persiaplay'),
    password: config.get<string>('DB_PASSWORD', 'persiaplay123'),
    database: config.get<string>('DB_DATABASE', 'persiaplay_db'),
    entities: ENTITIES,
    synchronize: false,
