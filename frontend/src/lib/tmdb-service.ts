...
const TMDB_CONFIG = {
  ...
  cacheDuration: parseInt(process.env.NEXT_PUBLIC_CACHE_DURATION || "3600000", 10), // 1 hour
...
const rateLimiter = new RateLimiter(TMDB_CONFIG.rateLimit, TMDB_CONFIG.rateWindow);
...
class TMDBClient {
  ...
  async getPopularMovies(language: "en" | "fa" = "en", page = 1): Promise<TMDBResponse<TMDBMovie>> {
  ...
  async getPopularTVShows(language: "en" | "fa" = "en", page = 1): Promise<TMDBResponse<TMDBTVShow>> {
