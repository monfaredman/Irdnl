...
export default function IranianMoviesPage() {
	...
	// Fetch Iranian movies from TMDB
	const { data: iranianMovies, loading, error } = useTMDBDiscoverMovies("iranian", undefined, {
		language,
	});

	// Extract unique genres
	const allGenres = Array.from(
		new Set(iranianMovies?.flatMap((m) => m.genres || []) || [])
	);
				...
				<Stack spacing={6}>
					...
					>
						<Stack spacing={2}>
							...
							>
								{language === "fa"
									? "بهترین فیلم‌های سینمای ایران"
									: "The best of Iranian cinema"}
							</Typography>
						</Stack>
					...
					>
						{iranianMovies && iranianMovies.length > 0 ? (
							iranianMovies.map((movie) => (
