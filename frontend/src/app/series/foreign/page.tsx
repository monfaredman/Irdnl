...
export default function ForeignSeriesPage() {
	...
	// Fetch foreign series from TMDB
	const { data: foreignSeries, loading, error } = useTMDBDiscoverSeries("foreign", undefined, {
		language,
	});

	// Extract unique genres
	const allGenres = Array.from(
		new Set(foreignSeries?.flatMap((s) => s.genres || []) || [])
	);
		...
		<Box sx={{ minHeight: "100vh", py: 8 }}>
			<Container maxWidth="xl">
				<Stack spacing={6}>
					...
					>
						{foreignSeries && foreignSeries.length > 0 ? (
