...
import { useTMDBDiscoverSeries } from "@/hooks/useTMDB";
...
export default function IranianSeriesPage() {
	...
	// Fetch Iranian series from TMDB
	const { data: iranianSeries, loading, error } = useTMDBDiscoverSeries("iranian", undefined, {
		language,
	});

	// Extract unique genres
	const allGenres = Array.from(
		new Set(iranianSeries?.flatMap((s) => s.genres || []) || [])
	);
				...
				<Stack spacing={6}>
					...
					>
						<Stack spacing={2}>
							...
							>
								{language === "fa" ? "سریال ایرانی" : "Iranian Series"}
							</Typography>
							<Typography
								variant="body1"
								sx={{
					...
					>
						{iranianSeries && iranianSeries.length > 0 ? (
							iranianSeries.map((show) => (
								<MediaCard key={show.id} item={show} type="series" />
							))
						) : (
							<Box sx={{ ...glassStyle, p: 8, textAlign: "center", gridColumn: "1 / -1" }}>
								<Typography sx={{ color: glassColors.text.tertiary, fontSize: "1.125rem" }}>
									{language === "fa" ? "سریالی پیدا نشد" : "No series found"}
								</Typography>
							</Box>
						)}
					</Box>
