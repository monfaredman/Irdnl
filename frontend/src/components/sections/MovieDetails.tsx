		...
		<div className="grid gap-8 lg:grid-cols-3">
			<div className="lg:col-span-2 space-y-4">
				...
				<div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-wide text-white/60">
					...
					<span>
						{movie.languages.map((lng) => lng.toUpperCase()).join(" / ")}
					</span>
				</div>
			</div>
			<div className="rounded-3xl border border-white/10 bg-white/5 p-6">
				...
				<div className="mt-8 space-y-3 text-sm">
					<div>
						<h3 className="font-semibold uppercase tracking-wide text-white/50">
							Downloads
						</h3>
					...
					<div>
						...
						<ul className="mt-2 space-y-2 text-white/70">
							{movie.subtitles.map((subtitle) => (
								<li
									key={subtitle.id}
									className="flex items-center justify-between rounded-2xl bg-black/30 px-3 py-2"
								>
